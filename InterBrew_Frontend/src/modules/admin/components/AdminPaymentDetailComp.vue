    ...
    <v-menu>
      ...
      <v-list>
        <v-list-item
          ...
          :key="index"
          @click="getPaymentListByDate(item.title)"
        >
  ...
  <v-table>
    <thead>
      <tr>
        ...
        <th class="text-left">결제품목</th>
        <th class="text-left">결제수량</th>
    ...
    <tbody>
      <tr v-for="payment in paymentList" :key="payment.userId">
        ...
        <td>{{ payment.paymentItemCount }}개</td>
        <td>{{ payment.coffeePrice * payment.paymentItemCount }}원</td>
      </tr>
    </tbody>
  </v-table>
...
import { $getPaymentListByDate } from '@/api/payment';

export default {
  name: "AdminPaymentDetailComp",
  setup() {
    ...
    async function getPaymentListByDate(date){
      selectedDate.value = date;
      const formattedMonth = formatDateToFullString(date);
      console.log(formattedMonth);
      try {
          const res = await $getPaymentListByDate(formattedMonth);
    ...
    // function updateDateAndPayments(date) {
    //   selectedDate.value = date;
    //   getPaymentListByDate(date);
    // }
    ...
    onMounted(() => { getPaymentListByDate(selectedDate.value);});
